...
def conserved_TE(chro, gff, maf_dir, output):
    chro_id = 'MSU7.%s' %(chro)
    maf     = '%s/%s.axt.chain.prenet.net.axt.maf' %(maf_dir, chro)
    index   = maf_index(maf, chro_id)
    chro_te = '%s/%s.axt.chain.prenet.net.axt.maf.te_conservation' %(maf_dir, chro)
    ofile = open(chro_te, 'w')
...
def main():
    ...
    for chro in sorted(gff.keys()):
        #te_file = conserved_TE(chro, gff, args.maf)
        #files.append(te_file)
        print chro
        p = mp.Process(target=conserved_TE, args=(chro, gff, args.maf, output))
        processes.append(p)
        p.start()
    ...
    #get result from each process and merge
    files = []
    for p in processes:
        files.append(output.get())

    cmd = 'cat %s > %s' %(' '.join(sorted(files)), args.output)
    print cmd
    os.system(cmd)
